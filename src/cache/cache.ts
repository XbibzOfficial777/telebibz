...
export class MemoryCache implements Cache<string, unknown> {
  private readonly storage = new MemoryStorage<string, unknown>();
  private readonly namespace: string;
  constructor(namespace = "telebibz") { this.namespace = namespace; }
  private key(key: string): string { return `${this.namespace}:${key}`; }
  get(key: string): Promise<unknown> { return this.storage.get(this.key(key)); }
  set(key: string, value: unknown, ttlMs?: number): Promise<void> { return this.storage.set(this.key(key), value, ttlMs === undefined ? {} : { ttlMs }); }
  ...
  async getOrSet(key: string, factory: () => unknown | Promise<unknown>, ttlMs?: number): Promise<unknown> { const existing = await this.get(key); if (existing !== undefined) return existing; const value = await factory(); await this.set(key, value, ttlMs); return value; }
}
